...
expName = 'SMT'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\蔡振辛\\Desktop\\编程学习\\Psychopy\\psychopy-SMT\\SMT_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Identity = visual.TextStim(win=win, name='Identity',
        text='',
        font='Arial',
        pos=(0, -50), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    ...
    IntroComponents = [Intro_1, key_resp_Intro]
    ...
    Practice = data.TrialHandler(nReps=2.0, method='random', 
        ...
        trialList=data.importConditions('Stim/blankTemplate1.xlsx'),
    ...
    for thisPractice in Practice:
        ...
        key_resp.rt = []
        _key_resp_allKeys = []
        Identity.setText(Words)
        # keep track of which components have finished
        trialComponents = [image, key_resp, Identity]
        ...
        while continueRoutine and routineTimer.getTime() < 0.8:
            ...
            # if Identity is starting this frame...
            if Identity.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Identity.frameNStart = frameN  # exact frame index
                Identity.tStart = t  # local t and not account for scr refresh
                Identity.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Identity, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Identity.started')
                # update status
                Identity.status = STARTED
                Identity.setAutoDraw(True)
            
            # if Identity is active this frame...
            if Identity.status == STARTED:
                # update params
                pass
            
            # if Identity is stopping this frame...
            if Identity.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Identity.tStartRefresh + 0.8-frameTolerance:
                    # keep track of stop time/frame for later
                    Identity.tStop = t  # not accounting for scr refresh
                    Identity.tStopRefresh = tThisFlipGlobal  # on global time
                    Identity.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Identity.stopped')
                    # update status
                    Identity.status = FINISHED
                    Identity.setAutoDraw(False)
    ...
    # set up handler to look after randomisation of conditions etc
    Formal = data.TrialHandler(nReps=5.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Stim/blankTemplate1.xlsx'),
        seed=None, name='Formal')
    thisExp.addLoop(Formal)  # add the loop to the experiment
    thisFormal = Formal.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisFormal.rgb)
    if thisFormal != None:
        for paramName in thisFormal:
            globals()[paramName] = thisFormal[paramName]
    
    for thisFormal in Formal:
        currentLoop = Formal
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # pause experiment here if requested
        if thisExp.status == PAUSED:
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            ...
            if str(ans).lower() == 'none':
               ...
               key_resp.corr = 0;  # failed to respond (incorrectly)
        # store data for Formal (TrialHandler)
        Formal.addData('key_resp.keys',key_resp.keys)
        Formal.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            Formal.addData('key_resp.rt', key_resp.rt)
            Formal.addData('key_resp.duration', key_resp.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 5.0 repeats of 'Formal'
    ...
    # keep track of which components have finished
    EndComponents = [text, key_resp_end]
