...
expName = 'IAT(test02)'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\GitHub_programe\\GitHub\\Psychopy\\IAT\\IAT(test02).py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_resp_iso_Practice') is None:
        # initialise key_resp_iso_Practice
        key_resp_iso_Practice = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_iso_Practice',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    iso_stim = visual.TextStim(win=win, name='iso_stim',
        text='',
        font='Arial',
        pos=(0, 0), height=10.0, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    key_resp_iso_Practice = keyboard.Keyboard(deviceName='key_resp_iso_Practice')
    ...
    Instruction01Components = [Instruction_01, key_resp]
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    Instruction02Components = [Instruction_02, key_resp_2]
    ...
    Practice_iso = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Loop/Practice_iso.xlsx'),
        seed=None, name='Practice_iso')
    thisExp.addLoop(Practice_iso)  # add the loop to the experiment
    thisPractice_iso = Practice_iso.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisPractice_iso.rgb)
    if thisPractice_iso != None:
        for paramName in thisPractice_iso:
            globals()[paramName] = thisPractice_iso[paramName]
    
    for thisPractice_iso in Practice_iso:
        currentLoop = Practice_iso
        ...
        iso_stim.setText(Words)
        key_resp_iso_Practice.keys = []
        key_resp_iso_Practice.rt = []
        _key_resp_iso_Practice_allKeys = []
        # keep track of which components have finished
        PracticeComponents = [iso_stim, key_resp_iso_Practice]
        ...
        while continueRoutine and routineTimer.getTime() < 1.5:
            ...
            # if iso_stim is starting this frame...
            if iso_stim.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                iso_stim.frameNStart = frameN  # exact frame index
                iso_stim.tStart = t  # local t and not account for scr refresh
                iso_stim.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(iso_stim, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'iso_stim.started')
                # update status
                iso_stim.status = STARTED
                iso_stim.setAutoDraw(True)
            
            # if iso_stim is active this frame...
            if iso_stim.status == STARTED:
                # update params
                pass
            
            # if iso_stim is stopping this frame...
            if iso_stim.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > iso_stim.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    iso_stim.tStop = t  # not accounting for scr refresh
                    iso_stim.tStopRefresh = tThisFlipGlobal  # on global time
                    iso_stim.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'iso_stim.stopped')
                    # update status
                    iso_stim.status = FINISHED
                    iso_stim.setAutoDraw(False)
            
            # *key_resp_iso_Practice* updates
            waitOnFlip = False
            
            # if key_resp_iso_Practice is starting this frame...
            if key_resp_iso_Practice.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_iso_Practice.frameNStart = frameN  # exact frame index
                key_resp_iso_Practice.tStart = t  # local t and not account for scr refresh
                key_resp_iso_Practice.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_iso_Practice, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_iso_Practice.started')
                # update status
                key_resp_iso_Practice.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_iso_Practice.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_iso_Practice.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
            # if key_resp_iso_Practice is stopping this frame...
            if key_resp_iso_Practice.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_iso_Practice.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    key_resp_iso_Practice.tStop = t  # not accounting for scr refresh
                    key_resp_iso_Practice.tStopRefresh = tThisFlipGlobal  # on global time
                    key_resp_iso_Practice.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'key_resp_iso_Practice.stopped')
                    # update status
                    key_resp_iso_Practice.status = FINISHED
                    key_resp_iso_Practice.status = FINISHED
            if key_resp_iso_Practice.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_iso_Practice.getKeys(keyList=['e','i'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_iso_Practice_allKeys.extend(theseKeys)
                if len(_key_resp_iso_Practice_allKeys):
                    key_resp_iso_Practice.keys = _key_resp_iso_Practice_allKeys[-1].name  # just the last key pressed
                    key_resp_iso_Practice.rt = _key_resp_iso_Practice_allKeys[-1].rt
                    key_resp_iso_Practice.duration = _key_resp_iso_Practice_allKeys[-1].duration
                    # was this correct?
                    if (key_resp_iso_Practice.keys == str(ans)) or (key_resp_iso_Practice.keys == ans):
                        key_resp_iso_Practice.corr = 1
                    else:
                        key_resp_iso_Practice.corr = 0
                    # a response ends the routine
                    continueRoutine = False
        ...
        # check responses
        if key_resp_iso_Practice.keys in ['', [], None]:  # No response was made
            key_resp_iso_Practice.keys = None
            # was no response the correct answer?!
            if str(ans).lower() == 'none':
               key_resp_iso_Practice.corr = 1;  # correct non-response
            else:
               key_resp_iso_Practice.corr = 0;  # failed to respond (incorrectly)
        # store data for Practice_iso (TrialHandler)
        Practice_iso.addData('key_resp_iso_Practice.keys',key_resp_iso_Practice.keys)
        Practice_iso.addData('key_resp_iso_Practice.corr', key_resp_iso_Practice.corr)
        if key_resp_iso_Practice.keys != None:  # we had a response
            Practice_iso.addData('key_resp_iso_Practice.rt', key_resp_iso_Practice.rt)
            Practice_iso.addData('key_resp_iso_Practice.duration', key_resp_iso_Practice.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    # completed 1.0 repeats of 'Practice_iso'
