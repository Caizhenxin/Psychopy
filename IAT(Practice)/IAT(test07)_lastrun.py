...
expName = 'IAT(test07)'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\GitHub_programe\\GitHub\\Psychopy\\IAT\\IAT(test07)_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_resp_t2_Practice') is None:
        # initialise key_resp_t2_Practice
        key_resp_t2_Practice = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_t2_Practice',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_5 = visual.TextStim(win=win, name='text_5',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_t2_Practice = keyboard.Keyboard(deviceName='key_resp_t2_Practice')
    
    # --- Initialize components for Routine "Feedback_t2_reverse" ---
    fb_7 = visual.TextStim(win=win, name='fb_7',
        text='',
        font='Open Sans',
        pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    Practice_01 = data.TrialHandler(nReps=0.0, method='random', 
    ...
    # completed 0.0 repeats of 'Practice_01'
    ...
    # set up handler to look after randomisation of conditions etc
    Practice_02 = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisPractice_02 in Practice_02:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Practice_02'
    ...
    # set up handler to look after randomisation of conditions etc
    Practice_T1 = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisPractice_T1 in Practice_T1:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Practice_T1'
    ...
    # set up handler to look after randomisation of conditions etc
    Formal_T1 = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Loop/Practice_t1.xlsx'),
    ...
    for thisFormal_T1 in Formal_T1:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Formal_T1'
    ...
    # set up handler to look after randomisation of conditions etc
    Practice_iso_Reverse = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisPractice_iso_Reverse in Practice_iso_Reverse:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Practice_iso_Reverse'
    ...
    # set up handler to look after randomisation of conditions etc
    Practice_t2_Reverse = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Loop/Practice_t2.xlsx'),
        seed=None, name='Practice_t2_Reverse')
    thisExp.addLoop(Practice_t2_Reverse)  # add the loop to the experiment
    thisPractice_t2_Reverse = Practice_t2_Reverse.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisPractice_t2_Reverse.rgb)
    if thisPractice_t2_Reverse != None:
        for paramName in thisPractice_t2_Reverse:
            globals()[paramName] = thisPractice_t2_Reverse[paramName]
    
    for thisPractice_t2_Reverse in Practice_t2_Reverse:
        currentLoop = Practice_t2_Reverse
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # pause experiment here if requested
        if thisExp.status == PAUSED:
            pauseExperiment(
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Practice_t2_reverse.started', globalClock.getTime(format='float'))
        text_5.setText(Words_t2)
        key_resp_t2_Practice.keys = []
        key_resp_t2_Practice.rt = []
        _key_resp_t2_Practice_allKeys = []
        # keep track of which components have finished
        Practice_t2_reverseComponents = [text_5, key_resp_t2_Practice]
        for thisComponent in Practice_t2_reverseComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Practice_t2_reverse" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.5:
            # get current time
            t = routineTimer.getTime()
            ...
            # if key_resp_t2_Practice is starting this frame...
            if key_resp_t2_Practice.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_t2_Practice.frameNStart = frameN  # exact frame index
                key_resp_t2_Practice.tStart = t  # local t and not account for scr refresh
                key_resp_t2_Practice.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_t2_Practice, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_t2_Practice.started')
                # update status
                key_resp_t2_Practice.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_t2_Practice.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_t2_Practice.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
            # if key_resp_t2_Practice is stopping this frame...
            if key_resp_t2_Practice.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_t2_Practice.tStartRefresh + 1.5-frameTolerance:
                    # keep track of stop time/frame for later
                    key_resp_t2_Practice.tStop = t  # not accounting for scr refresh
                    key_resp_t2_Practice.tStopRefresh = tThisFlipGlobal  # on global time
                    key_resp_t2_Practice.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'key_resp_t2_Practice.stopped')
                    # update status
                    key_resp_t2_Practice.status = FINISHED
                    key_resp_t2_Practice.status = FINISHED
            if key_resp_t2_Practice.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_t2_Practice.getKeys(keyList=['e','i'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_t2_Practice_allKeys.extend(theseKeys)
                if len(_key_resp_t2_Practice_allKeys):
                    key_resp_t2_Practice.keys = _key_resp_t2_Practice_allKeys[-1].name  # just the last key pressed
                    key_resp_t2_Practice.rt = _key_resp_t2_Practice_allKeys[-1].rt
                    key_resp_t2_Practice.duration = _key_resp_t2_Practice_allKeys[-1].duration
                    # was this correct?
                    if (key_resp_t2_Practice.keys == str(ans_t2)) or (key_resp_t2_Practice.keys == ans_t2):
        ...
        for thisComponent in Practice_t2_reverseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Practice_t2_reverse.stopped', globalClock.getTime(format='float'))
        # check responses
        if key_resp_t2_Practice.keys in ['', [], None]:  # No response was made
            key_resp_t2_Practice.keys = None
            # was no response the correct answer?!
        ...
        # store data for Practice_t2_Reverse (TrialHandler)
        Practice_t2_Reverse.addData('key_resp_t2_Practice.keys',key_resp_t2_Practice.keys)
        Practice_t2_Reverse.addData('key_resp_t2_Practice.corr', key_resp_t2_Practice.corr)
        if key_resp_t2_Practice.keys != None:  # we had a response
            Practice_t2_Reverse.addData('key_resp_t2_Practice.rt', key_resp_t2_Practice.rt)
            Practice_t2_Reverse.addData('key_resp_t2_Practice.duration', key_resp_t2_Practice.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Feedback_t2_reverse.started', globalClock.getTime(format='float'))
        ...
        fb_7.setColor(fb_col, colorSpace='rgb')
        fb_7.setText(fb_text)
        # keep track of which components have finished
        Feedback_t2_reverseComponents = [fb_7]
        for thisComponent in Feedback_t2_reverseComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Feedback_t2_reverse" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            # get current time
            t = routineTimer.getTime()
            ...
            # if fb_7 is starting this frame...
            if fb_7.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fb_7.frameNStart = frameN  # exact frame index
                fb_7.tStart = t  # local t and not account for scr refresh
                fb_7.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fb_7, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fb_7.started')
                # update status
                fb_7.status = STARTED
                fb_7.setAutoDraw(True)
            
            # if fb_7 is active this frame...
            if fb_7.status == STARTED:
                # update params
                pass
            
            # if fb_7 is stopping this frame...
            if fb_7.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fb_7.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    fb_7.tStop = t  # not accounting for scr refresh
                    fb_7.tStopRefresh = tThisFlipGlobal  # on global time
                    fb_7.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fb_7.stopped')
                    # update status
                    fb_7.status = FINISHED
        ...
        for thisComponent in Feedback_t2_reverseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Feedback_t2_reverse.stopped', globalClock.getTime(format='float'))
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Practice_t2_Reverse'
    ...
    # set up handler to look after randomisation of conditions etc
    Formal_t2_reverse = data.TrialHandler(nReps=0.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Loop/Practice_t2.xlsx'),
        seed=None, name='Formal_t2_reverse')
    thisExp.addLoop(Formal_t2_reverse)  # add the loop to the experiment
    thisFormal_t2_reverse = Formal_t2_reverse.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisFormal_t2_reverse.rgb)
    if thisFormal_t2_reverse != None:
        for paramName in thisFormal_t2_reverse:
            globals()[paramName] = thisFormal_t2_reverse[paramName]
    
    for thisFormal_t2_reverse in Formal_t2_reverse:
        currentLoop = Formal_t2_reverse
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # pause experiment here if requested
        if thisExp.status == PAUSED:
            pauseExperiment(
        ...
        # store data for Formal_t2_reverse (TrialHandler)
        Formal_t2_reverse.addData('key_resp_t2.keys',key_resp_t2.keys)
        Formal_t2_reverse.addData('key_resp_t2.corr', key_resp_t2.corr)
        if key_resp_t2.keys != None:  # we had a response
            Formal_t2_reverse.addData('key_resp_t2.rt', key_resp_t2.rt)
            Formal_t2_reverse.addData('key_resp_t2.duration', key_resp_t2.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 0.0 repeats of 'Formal_t2_reverse'
